...
// 사용법: SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node scripts/import-content.mjs [JSON경로] [--status published] [--dry-run]
...
function validateContent(content) {
  ...
  for (const exam of content.exams ?? []) {
    if (!exam.id || !exam.title || !exam.shortTitle) {
      errors.push(`시험 필수 필드 누락: ${JSON.stringify(exam.id ?? exam.title)}`);
      continue;
    }
  ...
  for (const question of content.questions ?? []) {
    ...
    if (!question.id || !question.examId || !question.subject || !question.prompt) {
...
async function findDbDuplicates(supabase, questions) {
  ...
  const { data, error } = await supabase
    ...
    .in('exam_id', examIds);
  if (error) {
    console.error(`기존 문항 조회 실패: ${error.message}`);
    process.exit(1);
  }
  ...
  for (const question of questions) {
    const existingId = existingByKey.get(`${question.examId}:${normalizeText(question.prompt)}`);
...
const { error: examError } = await supabase.from('exams').upsert(examRows, { onConflict: 'id' });
if (examError) {
...
const subjectRows = content.exams.flatMap((exam) =>
  exam.subjects.map((name, index) => ({ exam_id: exam.id, name, sort_order: index })),
);
